...
func main() {
	...
	if err != nil {
		...
		fmt.Println(os.Getenv("NEWRELIC_LICENSE_KEY"))
		panic(err)
	}
...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		// paging
		err := tx.Select(&items,
			"SELECT * FROM (SELECT * FROM `items` WHERE `seller_id` = ? AND `status` IN ('on_sale','trading','sold_out','cancel','stop') UNION SELECT * FROM `items` WHERE `buyer_id` = ? AND `status` IN ('on_sale','trading','sold_out','cancel','stop')) t WHERE created_at < ? OR (created_at <= ? AND id < ?) ORDER BY `created_at` DESC, `id` DESC LIMIT ?",
			user.ID,
			user.ID,
			time.Unix(createdAt, 0),
			time.Unix(createdAt, 0),
